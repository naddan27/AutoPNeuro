...
from rano_function_v2 import optimized_rano, rano, rano2

def get_rapno(mask,vox_x,vox_z,pic_dir,background, num_lesions, make_csv=True, print_output = False, get_volume = False, all_lesions = False):
	...
	for i in range(1,len(np.unique(conn_comp))):
		if print_output:
			print("Unique Connected Component ", i, "/", num_i)
		...
		for j in list(np.sum(curr_mask,(0,1)).argsort()[::-1]):
			...
			if np.sum(curr_mask[...,j]>0):
				jx += 1
				if print_output:
					print("Calculating slice ", jx, "/", np.sum(has_area_ix))
				...
				if pic_dir == None:
					curr_maj[j], curr_min[j] = rano(np.swapaxes(curr_mask[...,j],0,1), output_file=None, background_image=None, vox_x = vox_x, thres = thres, print_output = print_output)
				else:
					curr_maj[j], curr_min[j] = rano(np.swapaxes(curr_mask[...,j],0,1), output_file=pic_dir+str(i)+'_'+str(j)+'.png', background_image=np.swapaxes(background[...,j],0,1), vox_x = vox_x, thres = thres, get_volume = get_volume)
			# pbar.update(1)
...
def get_rapno2(mask,vox_x,vox_z,pic_dir,background, num_lesions, make_csv=True, print_output = False):
	#split connected components_____________________________________________
	#points are considered connected if they have the same value and are neighbors
	#with connectivity =3, neighbors are defined like as a 3x3 cube where the point is
	#center cube. All connected points are labeled with the same value
	...
	for i in range(1,len(np.unique(conn_comp))):
		...
		for j in list(np.sum(curr_mask,(0,1)).argsort()[::-1]):
			...
			if np.sum(curr_mask[...,j]>0):
				...
				# - pic file name: base_pic_dir + index of slice + index of connected component
				if pic_dir == None:
					curr_maj[j], curr_min[j] = rano2(np.swapaxes(curr_mask[...,j],0,1), output_file=None, background_image=None, vox_x = vox_x, thres = thres, print_output = print_output)
				else:
					curr_maj[j], curr_min[j] = rano2(np.swapaxes(curr_mask[...,j],0,1), output_file=pic_dir+str(i)+'_'+str(j)+'.png', background_image=np.swapaxes(background[...,j],0,1), vox_x = vox_x, thres = thres)
			# pbar.update(1)
...
def get_rapno_w_progress(mask,vox_x,vox_z,pic_dir,background, num_lesions, make_csv=True):
	#split connected components_____________________________________________
	#points are considered connected if they have the same value and are neighbors
	#with connectivity =3, neighbors are defined like as a 3x3 cube where the point is
	#center cube. All connected points are labeled with the same value
	...
	num_j = len(list(np.sum(conn_comp,(0,1)).argsort()[::-1]))
	num_i = len(np.unique(conn_comp)) - 1
	# pbar = tqdm(total = num_j * num_i, desc = "Calculating Individual RAPNO Score", bar_format= "{l_bar}{bar}{r_bar}")
	for i in range(1,len(np.unique(conn_comp))):
		print("Unique Connected Component ", i, "/", len(np.unique(conn_comp)))
		#make a mask with only the current connected component
		curr_mask = np.zeros(conn_comp.shape)
		idx = np.where(conn_comp==np.unique(conn_comp)[i]) #index where the connected component mask equals the component of interest
		curr_mask[idx] = 1.0
		...
		for j in list(np.sum(curr_mask,(0,1)).argsort()[::-1]):
			...
			if np.sum(curr_mask[...,j]>0):
				...
				print("Calculating slice ", jx, "/", np.sum(has_area_ix))
				if pic_dir == None:
					curr_maj[j], curr_min[j] = optimized_rano(np.swapaxes(curr_mask[...,j],0,1), output_file=None, background_image=None, vox_x = vox_x, thres = thres)
				else:
					curr_maj[j], curr_min[j] = optimized_rano(np.swapaxes(curr_mask[...,j],0,1), output_file=pic_dir+str(i)+'_'+str(j)+'.png', background_image=np.swapaxes(background[...,j],0,1), vox_x = vox_x, thres = thres)
			# pbar.update(1)
...
def optimized_get_rapno(mask,vox_x,vox_z,pic_dir,background, num_lesions, make_csv=True):
	#split connected components_____________________________________________
	#points are considered connected if they have the same value and are neighbors
	#with connectivity =3, neighbors are defined like as a 3x3 cube where the point is
	#center cube. All connected points are labeled with the same value
